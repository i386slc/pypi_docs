...
| Классы                                             | Описание                                                                                                                                                                                            |
| -------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| And(\*validators\[, error])                        | Составьте несколько валидаторов и объедините их сообщения об ошибках.                                                                                                                               |
| ContainsNoneOf(iterable, \*\[, error])             | Валидатор, который терпит неудачу, если **value** является последовательностью, и любой элемент в последовательности является членом последовательности, переданной как **iterable**.               |
| ContainsOnly(choices\[, labels, error])            | Валидатор, который завершается успешно, если **value** является последовательностью, и каждый элемент в последовательности также находится в последовательности, переданной в качестве **choices**. |
| Email(\*\[, error])                                | Проверяет адрес электронной почты.                                                                                                                                                                  |
| Equal(comparable, \*\[, error])                    | Валидатор, который завершается успешно, если переданное ему значение равно сравниваемому **comparable**.                                                                                            |
| Length(\[min, max, equal, error])                  | Валидатор, который завершается успешно, если переданное ему значение имеет длину между минимумом и максимумом.                                                                                      |
| NoneOf(iterable, \*\[, error])                     | Валидатор, который дает сбой, если значение является членом **iterable**.                                                                                                                           |
| OneOf(choices\[, labels, error])                   | Валидатор, который завершается успешно, если значение является элементом **choices**.                                                                                                               |
| Predicate(method, \*\[, error])                    | Вызывает указанный **method** объекта значения **value**.                                                                                                                                           |
| Range(\[min, max, min\_inclusive, ...])            | Валидатор, который завершается успешно, если переданное ему значение находится в пределах указанного диапазона.                                                                                     |
| Regexp(regex\[, flags, error])                     | Валидатор, который завершается успешно, если значение соответствует регулярному выражению **regex**.                                                                                                |
| URL(\*\[, relative, schemes, require\_tld, error]) | Проверяет URL-адрес.                                                                                                                                                                                |
| Validator()                                        | Абстрактный базовый класс для валидаторов.                                                                                                                                                          |
...
* **iterable** (iterable) - То же, что и NoneOf.
* **error** (str) - То же, что и NoneOf.
...
* **choices** (iterable) - То же, что и OneOf.
* **labels** (iterable) - То же, что и OneOf.
* **error** (str) - То же, что и OneOf.
